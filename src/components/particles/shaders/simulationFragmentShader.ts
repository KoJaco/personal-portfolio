...
uniform sampler2D positionsSphere;
...
void main() {
    ...
    float t = uTime * 0.015;
    vec3 pos = texture2D(positionsSphere, vUv).rgb; // basic simulation: displays the particles in place.
    vec3 curlPos = texture2D(positionsSphere, vUv).rgb;

    pos = curlNoise(pos * uCurlFreq + t);
    curlPos = curlNoise(curlPos * uCurlFreq + t);

    // small steps?
    // curlPos += curlNoise(curlPos * uCurlFreq) * 0.25;

    curlPos += curlNoise(curlPos * uCurlFreq * 2.0) * 0.5;
    
    // Very expensive work below
